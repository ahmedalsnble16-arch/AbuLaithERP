...
class DatabaseHelper {
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    // ============ جداول المعرض ============
    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableShowroomDailyEntries} (
        id TEXT PRIMARY KEY,
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableShowroomDailyAccount} (
        id TEXT PRIMARY KEY,
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableShowroomDailyExpenses} (
        id TEXT PRIMARY KEY,
        business_date TEXT NOT NULL,
        ...
        created_by TEXT,
        device_id TEXT,
        FOREIGN KEY (business_date) REFERENCES ${DBConstants.tableShowroomDailyAccount}(business_date)
      )
    ''');

    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableShowroomKhat} (
        id TEXT PRIMARY KEY,
        business_date TEXT NOT NULL,
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableWorkerAttendance} (
        id TEXT PRIMARY KEY,
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableWorkerDailyExpenses} (
        id TEXT PRIMARY KEY,
        worker_id TEXT NOT NULL,
        date TEXT NOT NULL,
    ...
    await db.execute('''
      CREATE TABLE ${DBConstants.tableTreasury} (
        ...
        source_module TEXT,
        source_id TEXT,
        payment_method TEXT DEFAULT '${DBConstants.paymentCash}',
        note TEXT,
        transaction_date TEXT NOT NULL,
        status TEXT DEFAULT '${DBConstants.statusApproved}',
        approved_by TEXT,
    ...
    // ============ إدارة النظام (Dynamic Configurations & Archives) ============
    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableDynamicConfigurations} (
        id TEXT PRIMARY KEY,
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS ${DBConstants.tableSystemArchives} (
        id TEXT PRIMARY KEY,
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 7) {
      await db.execute(
        'ALTER TABLE ${DBConstants.tableShowroomDailyExpenses} ADD COLUMN category TEXT DEFAULT "expense"'
      );
    }
    if (oldVersion < 8) {
      await db.execute(
    ...
    // ============ ترقية القاعدة للإصدار 13 (تنشيط الجداول الديناميكية والأرشيف) ============
    if (oldVersion < 13) {
      await db.execute('''
        CREATE TABLE IF NOT EXISTS ${DBConstants.tableDynamicConfigurations} (
          id TEXT PRIMARY KEY,
      ...
      for (var config in dynamicConfigs) {
        await db.insert(DBConstants.tableDynamicConfigurations, config);
      }
    }
  }
